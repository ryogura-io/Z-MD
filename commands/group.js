...
demote: {
    ...
    groupinfo: {
        ...
        execute: async (context) => {
            ...
            if (!isGroup) {
                await bot.sendMessage(chatId, { text: '❌ This command can only be used in groups.' });
                return;
            }
            ...
            try {
                ...
                try {
                    ...
                    console.warn('No group profile picture found or failed to fetch.');
                }

                if (groupProfilePic) {
                    // Download the image as buffer
                    const response = await require('axios').get(groupProfilePic, { responseType: 'arraybuffer' });
                    ...
                    await bot.sendMessage(chatId, { text: infoText });
                }

            } catch (error) {
                console.error('❌ Error getting group info:', error);
                await bot.sendMessage(chatId, { text: '❌ Error getting group information.' });
            }
        }
    },
